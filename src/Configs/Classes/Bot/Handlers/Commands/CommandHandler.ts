...
export default class CommandHandler {
    ...
    private isCommandDifferent(
        ...
        for (let i = 0; i < (commandData.options ?? []).length; i++) {
            ...
            if (
                // I don't know why, but I don't think this is being parsed correctly, if there's more than one subCommandGroup, it'll bug out and output undefined values.
                !existingOption ||
                !existingOption.type ||
                !existingOption.name ||
                !existingOption.description
            ) {
                this.logDebug("red", "Command Handler", `Invalid Option for command ${existingCommand.name}: ${existingOption}`, "error");
                throw new Error();
            }
